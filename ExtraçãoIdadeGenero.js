function importarFacebookAdsIdadeGenero() {
  const token = "token aqui ";
  const accountId = "ID da sua conta";
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("IDADE_GENERO") || SpreadsheetApp.getActiveSpreadsheet().insertSheet("IDADE_GENERO");
  

  const headers = ["Data", "Ad ID", "Ad Set ID","Amount spent", "Messaging Conversations Started", "Idade", "Gênero"];
  ...
  while (dataAtual <= hoje) {
    ...
    const url = `https://graph.facebook.com/v18.0/act_${accountId}/insights?fields=ad_id,adset_id,spend,actions&breakdowns=age,gender&level=ad&limit=1000&time_range=${encodeURIComponent(`{"since":"${since}","until":"${until}"}`)}&access_token=${token}`;
